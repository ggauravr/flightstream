...
## 🏗️ Core Package (@ggauravr/arrow-flight-server-node-core)
...
## 📊 CSV Adapter Package (@ggauravr/arrow-flight-server-node-csv-adapter)
...
## 🔧 Utils Package (@ggauravr/arrow-flight-server-node-utils)
...
**Example:**
```javascript
import { inferSchema } from '@ggauravr/arrow-flight-server-node-utils';
...
**Example:**
```javascript
import { createBatchStream } from '@ggauravr/arrow-flight-server-node-utils';
...
```javascript
import { FlightClient } from '@ggauravr/arrow-flight-server-node-examples/test-client';
